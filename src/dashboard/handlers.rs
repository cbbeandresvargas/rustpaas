...
fn error_page(code: u16, message: impl Into<String>, user: Option<User>) -> Response {
...
/// GET / — Landing page
pub async fn landing(OptionalAuthUser(user): OptionalAuthUser) -> Response {
    if user.is_some() {
...
pub async fn docs(OptionalAuthUser(user): OptionalAuthUser) -> Response {
    render(DocsTemplate {
        user,
        paas_version: PAAS_VERSION,
    })
...
pub async fn index(AuthUser(user): AuthUser, State(state): State<AppState>) -> Response {
    let projects = match Project::find_all(&state.db).await {
        Ok(p) => p,
        Err(e) => {
            error!("Failed to load projects: {}", e);
            return error_page(500, format!("Failed to load projects: {}", e), Some(user));
        }
    };
...
pub async fn project_detail(
    ...
    let project = match Project::find_by_id(&state.db, &id).await {
        Ok(Some(p)) => p,
        Ok(None) => return error_page(404, format!("Project '{}' not found", id), Some(user)),
        Err(e) => return error_page(500, format!("DB error: {}", e), Some(user)),
    };
